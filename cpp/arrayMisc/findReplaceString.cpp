...
string findReplaceString(string S, vector<int>& indexes, vector<string>& sources, vector<string>& targets) {
    ...
    // sort replacement indexes 
    for (int i = 0; i < indexes.size(); i++) {
        //replace[indexes[i]] = make_pair(sources[i], targets[i]);  // OK. Alternative
        replace.insert(make_pair(indexes[i], make_pair(sources[i], targets[i])));
    }
    
    int x = 0;
    for (auto &item : replace) {          
        int index = item.first;
        string s = item.second.first;
        string t = item.second.second;
        
        cout << index << "." << s << "." << t << endl;
        
        // Check if source exists in string, if not do nothing
        if (s != S.substr(index, s.length())) {
            continue;
        }
        
        // copy residual from last processed char of S to new index
        result += S.substr(x, index-x);
        // append current target to result string
        result += t;
        
        // record last char of S processed 
        x = index + s.length();
    }
    result += S.substr(x, S.length());
